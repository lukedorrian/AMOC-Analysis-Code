...
load(['regridded_dims_' modelName '_' num2str(spacing) '.mat']) %loads 0.25 grid from 49N to 67N (lat), -5 to -65 (long)
load(['sigma_and_MOC_' modelName '_' num2str(spacing) '_ds_' num2str(densitySpacing) '.mat'])
...
%%part to edit for switching models and spacing
if strcmp(modelName, 'CESM2')
    u_template = 'uo_Omon_CESM2_historical_r1i1p1f1_gn_ATL00000'; %need instance 1-4 + .nc
elseif strcmp(modelName, 'GISS')
    u_template = 'uo_Omon_GISS-E2-1-G_historical_r1i1p1f1_gn_'; 
elseif strcmp(modelName, 'IPSL')
    u_template = 'uo_Omon_IPSL-CM6A-LR_historical_r1i1p1f1_gn_';
end

sigma0_template_p1 = ['Sigma0_gridded_' modelName '_instance']; %then have instance number
sigma0_template_p2 = ['_' num2str(spacing) '.nc'];
Q_template_p1 = ['Q_V_gridded_' modelName '_instance']; %then have instance number
Q_template_p2 = ['_' num2str(spacing) '.nc'];

%going to assume positive = eastwards, negative is westward
if strcmp(modelName, 'CESM2')
    longU = ncread([u_template '1.nc'], 'lon');
    latU = ncread([u_template '1.nc'], 'lat'); %matches theta
    longU(longU > 180) = longU(longU>180) - 360;
elseif strcmp(modelName, 'GISS')
    longU = ncread([u_template '185001-187012.nc'], 'lon');
    latU = ncread([u_template '185001-187012.nc'], 'lat'); %matches theta
    [longU,latU] = meshgrid(longU,latU);
    longU = longU';
    latU = latU';
    longU(longU > 180) = longU(longU>180) - 360;
elseif strcmp(modelName,'IPSL')
    longU = double(ncread([u_template '185001-194912.nc'], 'lon'));
    latU = double(ncread([u_template '185001-194912.nc'], 'lat')); %matches theta
end

U_gridded(1:rGrid,1:cGrid,1:length(depth)) = 0;
Q_U_input = []; %for .nc file for future reference
...
for i = 0:modelNum %another thing to edit for model switching
 ...
 switch i
            ...
            case 4 %currently only applicable to GISS right now
   ...
   U = ncread(fn_U, 'uo');
   Q_V = ncread([Q_template_p1 num2str(i) Q_template_p2], 'Flow');
   Sigma0 = ncread([sigma0_template_p1 num2str(i) sigma0_template_p2], 'Sigma0');
   ...
   for j = 1:length(a)
        ...
        for z = 1:D %should probably just regrid first
         U_gridded(:,:,z) = griddata(longU,latU,U_specific(:,:,z), long, lat);

         %adding small correction for Gulf of St. Lawrence - to have
         %coherence between U and V Volumetric Flow Datasets
         U_copy = U_gridded(:,:,z);
         U_copy(combinedMask) = NaN;
         V_gridded(:,:,z) = U_copy;
         U_copy = [];
        end
        ...
        Q_U_gridded = U_gridded .* dA./(10^6); %convert to Sv	 
        ...
        Step1 = find(lat(:,1) == 50); %only focusing on flow on 50N, gulf of st. lawrence corrected
        ...
        Step2a = find(lat(:,1) >= 60.5 & lat(:,1) <= 64); %lat always a due to dimensioning of array
        Step2b = find(long(1,:) == -64.5);
        ...
        % Boundary
        Step3a = find(lat(:,1) == 64);
        Step3b = find(long(1,:) >= -65 & long(1,:) <= -51);
        ...
        Step4a = find(lat(:,1) == 65.5);
        Step4b = find(long(1,:) >= -40 & long(1,:) <= -23);
        ...
        Step5a = find(lat(:,1) == 64);
        Step5b = find(long(1,:) >= -16.5 & long(1,:) <= -7);
        ...
        Step6a = find(lat(:,1) >= 58 & lat(:,1) <= 64); %lat always 'a'/first due to dimensioning of array
        Step6b = find(long(1,:) == -7);
        ...
        %Step7a = find(lat(:,1) == diagonalLatitudeList(h)); %lat always 'a'/first due to dimensioning of array
...
Q_U_filename = ['Q_U_gridded_' modelName '_instance' num2str(i) '_' num2str(spacing) '.nc'];
disp(Q_U_filename)
...
% Flow_50N = seasonalFilter(Flow_50N);
% Flow_GI_NB = seasonalFilter(Flow_GI_NB);
% Flow_IF_EB = seasonalFilter(Flow_IF_EB);
% Flow_IF_NB = seasonalFilter(Flow_IF_NB);
% Flow_LS_NB = seasonalFilter(Flow_LS_NB);
% Flow_LS_WB = seasonalFilter(Flow_LS_WB);
...
inputFig1Title = ['VolumeInflow_' modelName '_' num2str(spacing) '.fig'];
...
inputFig2Title = ['VolumeBudget_' modelName '_' num2str(spacing) '.fig'];
savefig(inputFig2Title)

input1 = string(['Divergence_Calculations_' modelName '_' num2str(spacing) '.mat']);
input2 = string(['GI_NB_data_' modelName '_' num2str(spacing) '.mat']);
input3 = string(['LS_NB_data_' modelName '_' num2str(spacing) '.mat']);
input4 = string(['IF_NB_data_' modelName '_' num2str(spacing) '.mat']);
input5 = string(['IF_EB_data_' modelName '_' num2str(spacing) '.mat']);
...
%% Contour plot for Greenland/Iceland Northern Boundary Section
 ...
 input1 = ['GINB_Contour_' modelName '_' num2str(spacing) '.fig'];
 ...
 input2 = ['LSNB_Contour_' modelName '_' num2str(spacing) '.fig'];
 ...
 input3 = ['IFNB_Contour_' modelName '_' num2str(spacing) '.fig'];
 ...
 % input4 = ['IFEB_Contour_' modelName '_' num2str(spacing) '.fig'];
...
% function filtered = seasonalFilter(data) %assuming row vector and time actually is correct
% data_copy = data;
% for i = 1:12
%     data(i:12:end) = data(i:12:end) - mean(data(i:12:end)) + mean(data);
% end
% filtered = data;
% filtered = movmean(filtered,3*12);
% filtered(data_copy == 0) = 0;
% end
